...
namespace geode {
template <typename ObjectType> class EnergyTermCollection {
...
public:
  ...
  bool remove_energy_term(const uuid &id) {
    ...
    for (const uuid &set_id : term->targeted_set_ids()) {
      ...
      vec.erase(std::remove(vec.begin(), vec.end(), term), vec.end());
  ...
  [[nodiscard]] index_t size() const { return energy_terms_.size(); }

  [[nodiscard]] const EnergyTerm<ObjectType> &get(const uuid &id) const {
    auto it = energy_terms_.find(id);
    OPENGEODE_EXCEPTION(
        it != energy_terms_.end(),
  ...
  [[nodiscard]] const absl::flat_hash_map<
      uuid, std::shared_ptr<EnergyTerm<ObjectType>>> &
  all_terms() const {
    return energy_terms_;
  }

  [[nodiscard]] const std::vector<std::shared_ptr<EnergyTerm<ObjectType>>> &
  terms_for_set(const uuid &set_id) const {
    const auto it = set_to_terms_.find(set_id);
    OPENGEODE_EXCEPTION(it != set_to_terms_.end(),
                        "[EnergyTermCollection] - Object Subset (",
                        set_id.string(), ") does not have any energy term.");
    return it->second;
  }

  std::string string() const {
    auto message =
        absl::StrCat("EnergyTermCollection: ", energy_terms_.size(), " terms:");
