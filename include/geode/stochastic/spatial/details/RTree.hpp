// This code has been copied from https://github.com/nushoin/RTree
...
public:
    ...
    /// Function will reset, not append to array. \param a_resultCallback
    /// Callback function to return result.  Callback should return 'true' to
    ...
    /// \param a_min Min of search bounding rect
    /// \param a_max Max of search bounding rect
    /// \param a_resultCallback Callback function to return result.  The
    /// Callback takes both the resulting data point and the calculated square
    /// distance. Callback should return 'true' to continue searching \return
    /// Returns the number of entries found
    size_t NNSearch( const ELEMTYPE a_min[NUMDIMS],
        const ELEMTYPE a_max[NUMDIMS],
    ...
    private:
        ...
        {
            MAX_STACK = 32
        }; //  Max stack size. Allows almost n^32 where n is number of branches
           //  in node
...
protected:
    ...
    bool InsertRectRec(
        const Branch& a_branch, Node* a_node, Node** a_newNode, int a_level );
    ...
    void GetBranches(
        Node* a_node, const Branch* a_branch, PartitionVars* a_parVars );
    ...
    ELEMTYPEREAL m_unitSphereVolume; ///< Unit sphere constant for required
...
public:
    ...
    size_t Write( const TYPE& a_value )
    {
        RTREE_ASSERT( m_file );
        return fwrite(
            static_cast< void* >( &a_value ), sizeof( a_value ), 1, m_file );
    }
    ...
    {
        RTREE_ASSERT( m_file );
        return fwrite( static_cast< void* >( a_array ),
    ...
    {
        RTREE_ASSERT( m_file );
        return fread( static_cast< void* >( a_array ), sizeof( TYPE ) * a_count,
            1, m_file );
    }
};
...
RTREE_QUAL::RTree()
{
    ...
    {
        m_unitSphereVolume =
            static_cast< ELEMTYPEREAL >( UNIT_SPHERE_VOLUMES[NUMDIMS] );
    }
    else
    {
        ...
        m_unitSphereVolume = static_cast< ELEMTYPEREAL >(
            1.0 / std::sqrt( NUMDIMS * M_PI )
...
RTREE_TEMPLATE
size_t RTREE_QUAL::NNSearch( const ELEMTYPE a_min[NUMDIMS],
    const ELEMTYPE a_max[NUMDIMS],
    std::function< bool( const DATATYPE&, ELEMTYPE ) > callback ) const
{
    ...
    struct QueueItem
    {
        QueueItem( Branch* b, ELEMTYPE dist ) : branch( b ), distance( dist ) {}
...
void RTREE_QUAL::InitRect( Rect* a_rect )
{
    for( int index = 0; index < NUMDIMS; ++index )
    {
        a_rect->m_min[index] = static_cast< ELEMTYPEREAL >( 0 );
    ...
    const Branch& a_branch, Node* a_node, Node** a_newNode, int a_level )
{
    ...
    if( a_node->m_level > a_level )
    {
        ...
        if( !childWasSplit )
        {
            ...
            // with the existing bounding box
            a_node->m_branch[index].m_rect = CombineRect(
                &a_branch.m_rect, &( a_node->m_branch[index].m_rect ) );
            return false;
        }
        else
        {
            // Child was split. The old branches are now re-partitioned to two
            // nodes so we have to re-calculate the bounding boxes of each node
            ...
            // newly-created node to a_node as well. a_node might be split
            // because of that.
...
//
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRect(
    const Branch& a_branch, Node** a_root, int a_level )
{
    RTREE_ASSERT( a_root );
    ...
    {
        RTREE_ASSERT(
            a_branch.m_rect.m_min[index] <= a_branch.m_rect.m_max[index] );
    }
#endif //_DEBUG
...
int RTREE_QUAL::PickBranch( const Rect* a_rect, Node* a_node )
{
    ...
    ELEMTYPEREAL area;
    ELEMTYPEREAL bestArea = static_cast< ELEMTYPEREAL >( -1 );
    int best = 0;
    Rect tempRect;
...
RTREE_TEMPLATE
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(
    const Rect* a_rectA, const Rect* a_rectB )
{
...
RTREE_TEMPLATE
void RTREE_QUAL::SplitNode(
    Node* a_node, const Branch* a_branch, Node** a_newNode )
{
    RTREE_ASSERT( a_node );
    RTREE_ASSERT( a_branch );
    ...
    *a_newNode = AllocNode();
    ...
    RTREE_ASSERT(
        ( a_node->m_count + ( *a_newNode )->m_count ) == parVars->m_total );
}
...
ELEMTYPEREAL RTREE_QUAL::RectVolume( Rect* a_rect )
{
    ...
    RTREE_ASSERT( volume >= static_cast< ELEMTYPEREAL >( 0 ) );
...
ELEMTYPEREAL RTREE_QUAL::RectSphericalVolume( Rect* a_rect )
{
    ...
    for( int index = 0; index < NUMDIMS; ++index )
    {
        ELEMTYPEREAL halfExtent =
            ( static_cast< ELEMTYPEREAL >( a_rect->m_max[index] )
                - static_cast< ELEMTYPEREAL >( a_rect->m_min[index] ) )
            * static_cast< ELEMTYPEREAL >( 0.5 );
    ...
    {
        return static_cast< ELEMTYPEREAL >(
            pow( radius, NUMDIMS ) * m_unitSphereVolume );
    }
}
...
ELEMTYPEREAL RTREE_QUAL::CalcRectVolume( Rect* a_rect )
{
    ...
    return RectSphericalVolume(
        a_rect ); // Slower but helps certain merge cases
...
void RTREE_QUAL::ChoosePartition( PartitionVars* a_parVars, int a_minFill )
{
    ...
    while( ( ( a_parVars->m_count[0] + a_parVars->m_count[1] )
               < a_parVars->m_total )
           && ( a_parVars->m_count[0]
                < ( a_parVars->m_total - a_parVars->m_minFill ) )
           && ( a_parVars->m_count[1]
                < ( a_parVars->m_total - a_parVars->m_minFill ) ) )
    {
        firstTime = true;
        for( int index = 0; index < a_parVars->m_total; ++index )
        {
            if( PartitionVars::NOT_TAKEN == a_parVars->m_partition[index] )
            {
                ...
                ELEMTYPEREAL growth0 =
                    CalcRectVolume( &rect0 ) - a_parVars->m_area[0];
                ELEMTYPEREAL growth1 =
                    CalcRectVolume( &rect1 ) - a_parVars->m_area[1];
    ...
    RTREE_ASSERT( ( a_parVars->m_count[0] + a_parVars->m_count[1] )
                  == a_parVars->m_total );
...
RTREE_TEMPLATE
void RTREE_QUAL::LoadNodes(
    Node* a_nodeA, Node* a_nodeB, PartitionVars* a_parVars )
{
    ...
    for( int index = 0; index < a_parVars->m_total; ++index )
    {
        RTREE_ASSERT( a_parVars->m_partition[index] == 0
...
RTREE_TEMPLATE
void RTREE_QUAL::InitParVars(
    PartitionVars* a_parVars, int a_maxRects, int a_minFill )
{
    RTREE_ASSERT( a_parVars );

    a_parVars->m_count[0] = a_parVars->m_count[1] = 0;
    a_parVars->m_area[0] = a_parVars->m_area[1] =
        static_cast< ELEMTYPEREAL >( 0 );
    ...
    if( !RemoveRectRec( a_rect, a_id, *a_root, &reInsertList ) )
    {
        ...
        while( reInsertList )
        {
            ...
            for( int index = 0; index < tempNode->m_count; ++index )
            {
                ...
                InsertRect(
                    tempNode->m_branch[index], a_root, tempNode->m_level );
            }
        ...
        // replace if with while? In case there is a whole branch of redundant
        // roots...
...
// Returns 1 if record not found, 0 if success.
RTREE_TEMPLATE
bool RTREE_QUAL::RemoveRectRec(
    Rect* a_rect, const DATATYPE& a_id, Node* a_node, ListNode** a_listNode )
{
    ...
    if( a_node->IsInternalNode() ) // not a leaf node
    {
        ...
        {
            if( a_rect == NULL
                || Overlap( a_rect, &( a_node->m_branch[index].m_rect ) ) )
            {
                        ...
                        a_node->m_branch[index].m_child, a_listNode ) )
                {
                    ...
                    else
                    {
                        ...
                        DisconnectBranch(
                            a_node, index ); // Must return after this call as
                                             // count has changed
                    }
                    return false;
                }
...
// Decide whether two rectangles overlap.
RTREE_TEMPLATE
ELEMTYPE RTREE_QUAL::SquareDistance(
    Rect const& a_rectA, Rect const& a_rectB ) const
{
    ELEMTYPE dist{};
    ...
    for( int index = 0; index < NUMDIMS; ++index )
    {
        ...
        dist +=
            diff1 * diff2 < 0 ? 0 : std::min( diff1 * diff1, diff2 * diff2 );
    }
    return dist;
}
