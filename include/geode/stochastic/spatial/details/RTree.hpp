...
public:
  ...
  size_t
  NNSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
...
public:
  ...
  template <typename TYPE> size_t WriteArray(const TYPE *a_array, int a_count) {
    RTREE_ASSERT(m_file);
    return fwrite(static_cast<void *>(a_array), sizeof(TYPE) * a_count, 1,
                  m_file);
  }
  ...
  template <typename TYPE> size_t ReadArray(TYPE *a_array, int a_count) {
    RTREE_ASSERT(m_file);
    return fread(static_cast<void *>(a_array), sizeof(TYPE) * a_count, 1,
                 m_file);
  }
};
...
void RTREE_QUAL::CopyRec(Node *current, Node *other) {
  ...
  if (current->IsInternalNode()) // not a leaf node
  {
    for (int index = 0; index < current->m_count; ++index) {
      ...
      std::copy(otherBranch->m_rect.m_min, otherBranch->m_rect.m_min + NUMDIMS,
                currentBranch->m_rect.m_min);

      std::copy(otherBranch->m_rect.m_max, otherBranch->m_rect.m_max + NUMDIMS,
      ...
      if (a_rect == NULL ||
        ...
        if (!RemoveRectRec(a_rect, a_id, a_node->m_branch[index].m_child,
          ...
          if (a_node->m_branch[index].m_child->m_count >= MINNODES) {
            ...
            DisconnectBranch(a_node, index); // Must return after this call as
                                             // count has changed
          }
          return false;
        }
...
// Decide whether two rectangles overlap.
RTREE_TEMPLATE
ELEMTYPE RTREE_QUAL::SquareDistance(Rect const &a_rectA,
                                    Rect const &a_rectB) const {
  ELEMTYPE dist{};
