...
const ObjectSet<Type> &ObjectSets<Type>::get_set(const uuid &set_id) const {
...
template <typename Type>
std::vector<ObjectId>
ObjectSets<Type>::get_objects_in_set(const uuid &set_id) const {
...
template <typename Type> index_t ObjectSets<Type>::nb_sets() const {
  return sets_.size();
}
...
template <typename Type> index_t ObjectSets<Type>::nb_objects() const {
...
template <typename Type> uuid ObjectSets<Type>::add_set(std::string_view name) {
  ...
  OPENGEODE_EXCEPTION(inserted, "[ObjectSet]- group (", new_set_id.string(),
                      ") already exists.");
  return new_set_id;
}
...
template <typename Type>
void ObjectSets<Type>::update_free_object(const ObjectId &old_object_id,
                                          Type &&new_object) {
  OPENGEODE_EXCEPTION(!old_object_id.fixed,
...
void ObjectSets<Type>::remove_free_object(const ObjectId &object_id) {
  ...
  OPENGEODE_EXCEPTION(!object_id.fixed,
  ...
  ObjectId last_free_id{static_cast<geode::index_t>(set.nb_free_objects() - 1),
                        false, object_id.set_id};
  if (object_id != last_free_id) {
    const auto &last_free_obj = set.get_free_object(last_free_id.index);
...
template <typename Type>
std::vector<ObjectId>
ObjectSets<Type>::neighbors(const ObjectId &object_id,
  ...
  return neighborhood_.get_all_neighbor_ids(box, targeted_set_ids, object_id);
}

template <typename Type>
std::vector<ObjectId>
ObjectSets<Type>::neighbors(const Type &object,
                            const std::vector<uuid> &targeted_set_ids,
                            double searching_distance) const {
  auto box = object_bounding_box(object);
  box.extends(searching_distance * 2.);
  return neighborhood_.get_all_neighbor_ids(box, targeted_set_ids,
                                            std::nullopt);
}
...
template <typename Type> std::string ObjectSets<Type>::string() const {
  auto message = absl::StrCat("ObjectSets with ", nb_objects(), " objects in, ",
                              nb_sets(), " sets");
  for (const auto &[set_id, objs] : sets_) {
...
template class opengeode_stochastic_stochastic_api ObjectSets<OwnerSegment2D>;
} // namespace geode